...
class Student:
    def __init__(self,root):
        ...
        combo_gender['values']=("male","femaale","other")
    ...
    def add_students(self):
        if self.Roll_No_var.get()=="" or self.name_var.get()=="":
            ...
            values=(self.Roll_No_var.get(),self.name_var.get(),
                self.email_var.get(),self.gender_var.get(),self.contact_var.get(),
                self.dob_var.get(),self.txt_Address.get()
            )
    ...
    def get_cursor(self,ev):
        ...
        row=contents['values']
        self.Roll_No_var.self(row[0])
        self.name_var.self(row[1])
        self.email_var.self(row[2])
        self.gender_var.self(row[3])
        self.contact_var.self(row[4])
        self.dob_var.self(row[5])
        self.txt_Address.delete("1.0",END)
        self.txt_Address.insert(END,row[6])

    def update_data(self):
        con.execute("update students set name=%s,email=%s,gender=%s,contact=%s,dob=%s,address=%s where roll_no=%s",(self.name_var.get(),self.email_var.get(),self.gender_var.get(),self.dob_var.get(),self.txt_Address.get('1.0',END),self.Roll_No_var.get()))
        con.commit()
        self.fetch_data()
        self.clear()
        con.close()
